...
def padding(matrix, half_height=None):
    if half_height is None:
        ...
        max_lengths = [len(str(max(col, key=len))) for col in zip(*matrix[:half_height])]
...
for _ in range(height):
    row = list(map(lambda num: str(int(num)), sys.stdin.readline().split()))
    matrix.append(row)
if height * width < 1000000:
    matrix = padding(matrix)
else:
     matrix = padding(matrix, height // 30)
for row in matrix:
